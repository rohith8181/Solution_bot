    ...
    try {
        ...
        if (messageText.charAt(0) === '/') {
            ...
            switch (command) {
                case "start":
                    if (person) {
                        return sendMessage(messageObj, `Hello welcome ${messageObj.from.first_name}!\n- Your Available Credits :${person.Credits}`);
                    }
                    else {
                        await User.create({
                            AccountId: messageObj.from.id,
                            user_name: messageObj.from.first_name,
                            Credits: 1,
                        })
            ...
            return sendMessage(messageObj, "Sorry, I am not programmed to answer this query!");
        }
    }
    catch (err) {
        ...
        if (answerElement !== undefined && answerElement.length > 0) {
            ...
            if (person && person.AnswerFormat === "text") {
                ...
                answerElement.each(function () {
                    ...
                    combinedAnswer += "******------ANOTHER_USER_ANSWER-------*******"
                })
